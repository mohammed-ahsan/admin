...
const useCategorySubmit = (id, data) => {
  ...
  const [enabledForCatalog, setEnabledForCatalog] = useState(false);
    ...
    try {
      ...
      const categoryData = {
        ...
        description: {
          [language]:
            draftToHtml(
              convertToRaw(categoryDescription.getCurrentContent())
            ) || "",
        ...
        catalog: enabledForCatalog ? "show" : "hide",
        lang: language,
      };
      ...
      (async () => {
        try {
          ...
          if (res) {
            ...
            setEnabledForCatalog(res.catalog === "show" ? true : false);
            setCategoryDescription(
              createEditorState(showingTranslateValue(res.description))
            );
          }
        } catch (err) {
          notifyError(err ? err.response.data.message : err.message);
